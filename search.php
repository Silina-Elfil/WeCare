...
$query = "SELECT m.profilePicture, m.username, m.phoneNumber, dr.specialty, dr.location, dr.fees
                      ...
                      WHERE m.roleId = 2
                      AND (m.username LIKE '%$input%' OR dr.specialty LIKE '%$input%')";

$result = mysqli_query($con, $query);

if($result && mysqli_num_rows($result)>0){
    while($row = mysqli_fetch_assoc($result)){
        echo '<div class="col-lg-6 p-0">'; // Use col-lg-6 to display two cards in one row on large screens
        echo '<div class="card " style="max-width: 600px;">'; // Add mx-auto for horizontal centering
